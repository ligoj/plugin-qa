/*
 * Plugin "qa" — Quality Assurance (service-level).
 *
 * Parent of the qa-<tool> plugins (qa-sonarqube). The legacy controller
...
export default {
  ...
  feature(action, ...args) {
    ...
    if (!fn) throw new Error(`Plugin "qa" has no feature "${action}"`)
    return fn(...args)
  },
  service,
  meta: { icon: 'mdi-speedometer', color: 'green-darken-2' },
}
